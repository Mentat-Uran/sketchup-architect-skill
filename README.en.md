...
[中文主 README](README.md) · [Detailed operating contract](USAGE.md)
...
Then invoke it with `$sketchup-architect` or let Codex discover it automatically. If the directory already exists, use the safe update flow in the Chinese main README rather than overwriting it.
...
```text
Install or update the `sketchup-architect` Codex skill from
...
whether it has uncommitted changes. Stop instead of deleting or overwriting user
files if it is not a clean checkout. After installation, run:

  python3 scripts/offline_checks.py
  ruby scripts/session_contract_test.rb

Report the exact path, current Git revision, and check results. Do not copy the
